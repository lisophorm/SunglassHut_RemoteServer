...
//if(count($diff)>0) {
	...
	echo "total number of files:".count($xmlList)."<br/>";
...
function parseResult($xmlfile,$pic) {
	...
	echo("user has facebook and server result:".$server_output."<br/>");
	} else {
	unlink("/var/www/vhosts/ignitesocial.co.uk/httpdocs/summertimeremote/batchincoming/".$pic);
	unlink("/var/www/vhosts/ignitesocial.co.uk/httpdocs/summertimeremote/batchincoming/".$xmlfile);
	...
	$server_output = curl_exec ($ch);
	die("result of registration".$server_output);
	}
