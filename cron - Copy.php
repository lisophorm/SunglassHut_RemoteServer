<?php require_once('/var/www/vhosts/ignitesocial.co.uk/httpdocs/summertimeremote/Connections/localhost.php'); ?>
<?php
//
...
$rootDir="/var/www/vhosts/ignitesocial.co.uk/httpdocs/summertimeremote/";
...
function parseResult($xmlfile,$pic) {
	...
	if(strlen($token)>10) {
		echo "is facebook user<br/>";
		$type="fb";
	} else {
		$type="email";
	}
	
	$insertSQL = sprintf("INSERT IGNORE INTO stats (urn, friend_count,fb_id,post_type,email) VALUES (%s, %s,%s,%s,%s)",
  ...
  $Result1 = mysql_query($insertSQL, $localhost) or die("result=ERROR&message=".urlencode(mysql_error()));
	...
	echo "Insert new record";
 	$insertSQL = sprintf("INSERT IGNORE INTO users (urn, token, firstname, lastname, email, mobile, added, fb_id,customer,tickets,prize) VALUES (%s, %s, %s, %s,%s, %s, %s, %s,%s,%s,%s)",
                       GetSQLValueString($xml->urn, "text"),
                       GetSQLValueString($xml->token, "text"),
                       GetSQLValueString($xml->firstname, "text"),
                       GetSQLValueString($xml->lastname, "text"),
                       GetSQLValueString($xml->email, "text"),
                       GetSQLValueString($xml->mobile, "text"),
                       GetSQLValueString($xml->added, "text"),
					   GetSQLValueString($xml->fb_id, "text"),
					   GetSQLValueString($xml->VFCustomer, "text"),
					   GetSQLValueString($xml->ticketsWon, "text"),
					   GetSQLValueString($xml->grabPrize, "text")
					   );
		echo "query:".$insertSQL."<br/>";
  		mysql_select_db($database_localhost, $localhost);
  		$Result1 = mysql_query($insertSQL, $localhost) or die($insertSQL."-".mysql_error());
	...
	curl_setopt($ch, CURLOPT_URL,"http://luxottica.ignitesocial.co.uk/publishphoto.php");
	curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_POSTFIELDS,
				"urn=".$xml->urn."&file=".$pic);
	...
	$insertSQL = sprintf("update users set server_result=%s where urn=%s",
  		...
  		$Result1 = mysql_query($insertSQL, $localhost) or die(mysql_error());
			...
			curl_setopt($ch, CURLOPT_URL,"http://luxottica.ignitesocial.co.uk/publishscore.php");
			curl_setopt($ch, CURLOPT_POST, 1);
			curl_setopt($ch, CURLOPT_POSTFIELDS,
						"urn=".$xml->urn."&facebook=".$xml->facebook);
